...
namespace PureRpc.Transport.Kcp
{
    public abstract class KcpPeer
    {
        ...
        internal Kcp kcp;
        internal readonly string typeName = string.Empty;
        ...
        protected KcpPeer(KcpConfig config, uint cookie)
        {
            ...
            this.cookie = cookie;
            Log.Info($"[KCP] {typeName}: created with cookie={cookie}");
        ...
        void HandleTimeout(uint time)
        {
            ...
            if (time >= lastReceiveTime + timeout)
            {
                ...
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.Timeout, $"{typeName}: Connection timed out after not receiving any message for {timeout}ms. Disconnecting.");
                Disconnect();
            }
        }
        ...
        void HandleDeadLink()
        {
            ...
            if (kcp.state == -1)
            {
                // pass error to user callback. no need to log it manually.
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.Timeout, $"{typeName}: dead_link detected: a message was retransmitted {kcp.dead_link} times without ack. Disconnecting.");
                Disconnect();
            }
        }
        ...
        void HandleChoked()
        {
            ...
            if (total >= QueueDisconnectThreshold)
            {
                ...
                OnError(ErrorCode.Congestion,
                        $"{typeName}: disconnecting connection because it can't process data fast enough.\n" +
        ...
        bool ReceiveNextReliable(out KcpHeaderReliable header, out ArraySegment<byte> message)
        {
            ...
            if (msgSize > kcpMessageBuffer.Length)
            {
                ...
                // pass error to user callback. no need to log it manually.
                OnError(ErrorCode.InvalidReceive, $"{typeName}: possible allocation attack for msgSize {msgSize} > buffer {kcpMessageBuffer.Length}. Disconnecting the connection.");
                Disconnect();
                return false;
            }
            ...
            if (received < 0)
            {
                ...
                // pass error to user callback. no need to log it manually.
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.InvalidReceive, $"{typeName}: Receive failed with error={received}. closing connection.");
                Disconnect();
                return false;
            }
            ...
            {
                OnError(ErrorCode.InvalidReceive, $"{typeName}: Receive failed to parse header: {headerByte} is not defined in {typeof(KcpHeaderReliable)}.");
                Disconnect();
                return false;
            }
            ...
            if (ReceiveNextReliable(out KcpHeaderReliable header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    case KcpHeaderReliable.Hello:
                    {
                        ...
                        Log.Info($"[KCP] {typeName}: received hello with cookie={cookie}");
                    ...
                    case KcpHeaderReliable.Data:
                    {
                        ...
                        // pass error to user callback. no need to log it manually.
                        // typeName shows Server/ClientConn instead of just Connection.
                        OnError(ErrorCode.InvalidReceive, $"[KCP] {typeName}: received invalid header {header} while Connected. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                }
            ...
            while (ReceiveNextReliable(out KcpHeaderReliable header, out ArraySegment<byte> message))
            {
                ...
                switch (header)
                {
                    case KcpHeaderReliable.Hello:
                    {
                        ...
                        // typeName shows Server/ClientConn instead of just Connection.
                        Log.Warning($"{typeName}: received invalid header {header} while Authenticated. Disconnecting the connection.");
                        Disconnect();
                        break;
                    }
                    case KcpHeaderReliable.Data:
                    {
                        ...
                        else
                        {
                            // pass error to user callback. no need to log it manually.
                            // typeName shows Server/ClientConn instead of just Connection.
                            OnError(ErrorCode.InvalidReceive, $"{typeName}: received empty Data message while Authenticated. Disconnecting the connection.");
                            Disconnect();
                        }
                        break;
                    }
                    ...
                    case KcpHeaderReliable.Pong:
                    {
                        if (message.Count == 4)
                        {
                            ...
                            if (time >= originalTimestamp)
                            {
                                ...
                                // Log.Info($"[KCP] {typeName}: RTT={rttInMilliseconds}ms");
                            }
                        }
        ...
        public virtual void TickIncoming()
        {
            ...
            catch (SocketException exception)
            {
                ...
                // pass error to user callback. no need to log it manually.
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.ConnectionClosed, $"{typeName}: Disconnecting because {exception}. This is fine.");
                Disconnect();
            }
            ...
            catch (Exception exception)
            {
                ...
                // pass error to user callback. no need to log it manually.
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.Unexpected, $"{typeName}: unexpected Exception: {exception}");
                Disconnect();
            }
        }
        ...
        protected void OnRawInputReliable(ArraySegment<byte> message)
        {
            ...
            {
                // typeName shows Server/ClientConn instead of just Connection.
                Log.Warning($"[KCP] {typeName}: Input failed with error={input} for buffer with length={message.Count - 1}");
            }
        }
        ...
        protected void OnRawInputUnreliable(ArraySegment<byte> message)
        {
            ...
            {
                // 跳过无效包而非断开连接，提升抗干扰能力
                Log.Warning($"[KCP] {typeName}: ignoring unreliable message with invalid header byte={headerByte}");
                return;
            }
            ...
            switch (header)
            {
                case KcpHeaderUnreliable.Data:
                {
                    ...
                    else
                    {
                        ...
                        // Log.Info($"{typeName}: received unreliable message while not authenticated.");
                    }
                    break;
                }
                ...
                {
                    // typeName shows Server/ClientConn instead of just Connection.
                    Log.Info($"[KCP] {typeName}: received disconnect message");
                    Disconnect();
                    break;
                }
            }
        ...
        void SendReliable(KcpHeaderReliable header, ArraySegment<byte> content)
        {
            ...
            if (1 + content.Count > kcpSendBuffer.Length) // TODO
            {
                ...
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.InvalidSend, $"{typeName}: Failed to send reliable message of size {content.Count} because it's larger than ReliableMaxMessageSize={reliableMax}");
                return;
            }
            ...
            {
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.InvalidSend, $"{typeName}: Send failed with error={sent} for content with length={content.Count}");
            }
        }
        ...
        void SendUnreliable(KcpHeaderUnreliable header, ArraySegment<byte> content)
        {
            ...
            if (content.Count > unreliableMax)
            {
                // otherwise content is larger than MaxMessageSize. let user know!
                // typeName shows Server/ClientConn instead of just Connection.
                Log.Error($"[KCP] {typeName}: Failed to send unreliable message of size {content.Count} because it's larger than UnreliableMaxMessageSize={unreliableMax}");
                return;
            }
        ...
        public void SendHello()
        {
            ...
            // typeName shows Server/ClientConn instead of just Connection.
            Log.Info($"[KCP] {typeName}: sending handshake to other end with cookie={cookie}");
        ...
        public void SendData(ArraySegment<byte> data, KcpChannel channel)
        {
            ...
            if (data.Count == 0)
            {
                // pass error to user callback. no need to log it manually.
                // typeName shows Server/ClientConn instead of just Connection.
                OnError(ErrorCode.InvalidSend, $"{typeName}: tried sending empty message. This should never happen. Disconnecting.");
                Disconnect();
                return;
            }
        ...
        public virtual void Disconnect()
        {
            ...
            // typeName shows Server/ClientConn instead of just Connection.
            Log.Info($"[KCP] {typeName}: Disconnected.");
