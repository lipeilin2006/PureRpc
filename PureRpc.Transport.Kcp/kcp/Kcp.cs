...
namespace PureRpc.Transport.Kcp
{
    ...
    public class Kcp
    {
        ...
        internal readonly List<AckItem> acklist = new List<AckItem>(Math.Max(WND_RCV, 1024));
        ...
        public int Receive(byte[] buffer, int len)
        {
            ...
            while (rcv_queue.Count > 0)
            {
                ...
                Buffer.BlockCopy(seg.data, 0, buffer, offset, seg.length);
                offset += seg.length;
                len += seg.length;
            ...
            int removed = 0;
            int rcvCount = rcv_buf.Count;
            for (int i = 0; i < rcvCount; i++)
            {
                if (rcv_buf[i].sn == rcv_nxt && rcv_queue.Count < rcv_wnd)
                {
                    ++removed;
                    rcv_queue.Enqueue(rcv_buf[i]);
                    rcv_nxt++;
                }
                else
                {
        ...
        public int PeekSize()
        {
            ...
            if (seq.frg == 0) return seq.length;
        ...
        public void Flush()
        {
            ...
            // flush acknowledges
            int ackCount = acklist.Count;
            for (int ai = 0; ai < ackCount; ai++)
            {
                var ack = acklist[ai];
            ...
            // flush data segments
            int change = 0;
            int sndBufCount = snd_buf.Count;
            for (int si = 0; si < sndBufCount; si++)
            {
                Segment segment = snd_buf[si];
                bool needsend = false;
                ...
                if (needsend)
                {
                    ...
                    int need = OVERHEAD + segment.length;
                    ...
                    if (segment.length > 0)
                    {
                        Buffer.BlockCopy(segment.data, 0, buffer, size, segment.length);
                        size += segment.length;
                    }
        ...
        public uint Check(uint current_)
        {
            ...
            for (int i = 0; i < sndCount; i++)
            {
                int diff = Utils.TimeDiff(snd_buf[i].resendts, current_);
                if (diff <= 0)
                {
                    return current_;
                }
