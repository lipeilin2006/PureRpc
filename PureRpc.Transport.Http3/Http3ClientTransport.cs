...
internal sealed partial class Http3ClientTransport : IClientTransport
{
    private readonly Http3ClientOptions _options;
    private readonly ILogger<Http3ClientTransport> _logger;
    ...
    public Http3ClientTransport(IOptions<Http3ClientOptions> options, ILogger<Http3ClientTransport>? logger = null)
    {
        _options = options.Value ?? throw new ArgumentNullException(nameof(options));
        _logger = logger ?? NullLogger<Http3ClientTransport>.Instance;
    }
    ...
    {
        if (_httpClient == null) throw new IOException("HTTP/3 client is not connected.");
    ...
    #region Logging
    [LoggerMessage(EventId = 1203, Level = LogLevel.Information, Message = "[HTTP3] Client ready at {Url}")]
